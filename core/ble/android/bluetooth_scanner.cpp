...
void BluetoothScannerJni::startScan() {
    ...
    auto scanMethodID = env->GetMethodID(scannerClass, "startScan", "(Landroid/content/Context;)V");
...
std::unique_ptr<BleDevice> BluetoothScannerJni::getDeviceByAddress(std::string address) {
    ...
    try {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "BluetoothScannerJni", "Failed get device by address: %s", error.what());
    }
...
void BluetoothScannerJni::subscribeDeviceFound(
    ...
    auto deviceFoundCallbackObj = env->NewObject(deviceFoundCallbackClass, deviceFoundCallbackCtor,
                                                 (jlong) this);
