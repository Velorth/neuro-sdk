...
template <typename T>
double spectrum_power_normalized(float low_frequency, float high_frequency, const std::vector<T> &spectrum, float frequency_step) {
	...
	return std::accumulate(spectrum.begin() + start, spectrum.begin() + stop, 0.0, [](const auto &init, const auto &value) { return init + value * value; });
}
...
public:
    ...
    SpectrumPowerChannel(const std::vector<SpectrumChannelPtr> &channels, 
	...
	void setWindowDuration(double duration_seconds) noexcept {
		mWindowDuration = duration_seconds;
		mDurationStep = mWindowDuration - mWindowDuration * mOverlapping;
	}

	void setOverlappingCoefficient(double coefficient) noexcept {
		if (mOverlapping > 0.99) mOverlapping = 0.99;
		else if (mOverlapping < 0.01) mOverlapping = 0.01;
		mDurationStep = mWindowDuration - mWindowDuration * mOverlapping;
	}
