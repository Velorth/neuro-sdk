...
struct hamming_window {
	template <typename ForwardInputIterator, typename OutIterator>
	void operator()(ForwardInputIterator input_begin, ForwardInputIterator input_end, OutIterator output_begin) {
		auto N = std::distance(input_begin, input_end);
		decltype(N) n(0);
		while (input_begin != input_end) {
			*output_begin = (0.54 - 0.46 * std::cos(2 * Pi*n / (N - 1))) * (*input_begin);
			++n;
			++input_begin;
			++output_begin;
		}
...
struct blackman_window {
	...
	void operator()(ForwardInputIterator input_begin, ForwardInputIterator input_end, OutIterator output_begin) {
		...
		decltype(N) n(0);
		while (input_begin != input_end) {
			*output_begin = (0.42 - 0.5 * std::cos(2 * Pi*n / (N - 1)) + 0.08 * std::cos(4 * Pi*n / (N - 1))) * (*input_begin);
			++n;
			++input_begin;
			++output_begin;
		}
...
struct sine_window {
	...
	void operator()(ForwardInputIterator input_begin, ForwardInputIterator input_end, OutIterator output_begin) {
		...
		decltype(N) n(0);
		while (input_begin != input_end) {
			*output_begin = std::sin(Pi*n / (N - 1)) * (*input_begin);
			++n;
			++input_begin;
			++output_begin;
		}
